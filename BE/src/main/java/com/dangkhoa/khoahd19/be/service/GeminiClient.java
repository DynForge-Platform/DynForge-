...
public class GeminiClient {
    ...
    public String complete(String systemPrompt, String userPrompt, int maxTokens) {
        if (!isConfigured()) {
            throw new BadRequestException(
                    "Tính năng Gradora AI chưa được cấu hình. Vui lòng đặt gemini.api-key trong application.properties.");
        }
        ...
        for (String model : MODEL_CASCADE) {
            try {
                ...
                if (msg.contains("429") || msg.contains("RESOURCE_EXHAUSTED")) {
                    ...
                    log.error("Gradora AI call failed on model [{}]: {}", model, msg);
                    throw new BadRequestException("Không thể kết nối với Gradora AI: " + msg);
                }
            }
        }
    ...
    private Map<?, ?> callGemini(String model, Map<String, Object> body) {
        ...
        if (resp == null) {
            throw new BadRequestException("Gradora AI không trả về dữ liệu.");
        }
        return resp;
    }
